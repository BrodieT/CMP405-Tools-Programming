...
void SelectDialogue::SetObjectData(std::vector<SceneObject>* SceneGraph, int * selection)
{
	...
	for (int i = 0; i < numSceneObjects; i++)
	{
		...
		ListBoxEntry data;
		data.ID = m_sceneGraph->at(i).ID;
		itemID += std::to_wstring(m_sceneGraph->at(i).ID).c_str();
		CString itemName = ".   ";
		itemName += m_sceneGraph->at(i).name.c_str();
		CString dataEntry = itemID + itemName;
		data.Label = dataEntry;
		m_dataEntries.push_back(data);
		m_listBox.AddString(dataEntry);// listBoxEntry.c_str());
	}
}
...
void SelectDialogue::Select()
{
	...
	//Use the data entries vector to compare labels and access the correct ID
	for (int i = 0; i < m_dataEntries.size(); i++)
	{
		if (currentSelectionValue == m_dataEntries[i].Label)
		{
			*m_currentSelection = m_dataEntries[i].ID;
		}
	}
