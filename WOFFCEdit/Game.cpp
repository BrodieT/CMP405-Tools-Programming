...

{
	...
	//Give the camera access to inut commands
	m_camera.m_InputCommands = &m_InputCommands;
}
...
//This function returns a pointer to the current display chunk
DisplayChunk* Game::GetTerrain()
{
	return &m_displayChunk;
}

//This function handles mouse picking for objects and terrain 
int Game::MousePicking()
{
	//Detects if terrain editing is active to decide what picking method should be used
	if (!m_TerrainEditModeActive)
	{
		int selectedID = -1;
		...
		for (int i = 0; i < m_displayList.size(); i++)
		{
			...
			{
				if (!isnan(XMVectorGetX(pickingVector)) && !isnan(XMVectorGetY(pickingVector)) && !isnan(XMVectorGetZ(pickingVector)) && !isnan(XMVectorGetW(pickingVector))
					&& !isnan(XMVectorGetX(nearPoint)) && !isnan(XMVectorGetY(nearPoint)) && !isnan(XMVectorGetZ(nearPoint)) && !isnan(XMVectorGetW(nearPoint)))
				{

					//checking for ray intersection
					if (m_displayList[i].m_model.get()->meshes[y]->boundingBox.Intersects(nearPoint, pickingVector, pickedDistance))
					{
						Ids.push_back(m_displayList[i].m_ID);
						dists.push_back(pickedDistance);
		...
		//Find the closest object where multiple are detected through picking 
		//for more accurate results
		float closestDist = D3D10_FLOAT32_MAX;
		for (int j = 0; j < Ids.size(); j++)
		{
			closestDist = std::min(closestDist, dists[j]);

			if (closestDist == dists[j])
			{
				selectedID = Ids[j];
			}
		}
	...
	{
		//Create near and far vectors using the mouse screen position
		XMVECTOR nearPoint = Vector4(m_InputCommands.mouse_X, m_InputCommands.mouse_Y, 0.0f, 1.0f);
		XMVECTOR farPoint = Vector4(m_InputCommands.mouse_X, m_InputCommands.mouse_Y, 1.0f, 1.0f);

		//Project the near and far points into worldspace
		XMVECTOR origin = XMVector3Unproject(nearPoint, 0.0f, 0.0f, m_ScreenDimensions.right, m_ScreenDimensions.bottom, m_deviceResources->GetScreenViewport().MinDepth, m_deviceResources->GetScreenViewport().MaxDepth, m_projection, m_camera.GetViewMatrix(), XMMatrixIdentity());
		XMVECTOR destination = XMVector3Unproject(farPoint, 0.0f, 0.0f, m_ScreenDimensions.right, m_ScreenDimensions.bottom, m_deviceResources->GetScreenViewport().MinDepth, m_deviceResources->GetScreenViewport().MaxDepth, m_projection, m_camera.GetViewMatrix(), XMMatrixIdentity());

		//Calculate direction vector from projected worldspace vectors, and normalise
		Vector3 Dir = destination - origin;
		Dir.Normalize();

		//Use the calculated origin and direction vectors to pick the terrain
		m_displayChunk.PickTerrain(Vector3(origin), Vector3(Dir));
		return -1;
	}
